...
function createDynamicElements() {
    ...
    // 3. Fotoğraf Yükleyici (Loader)
    const card = document.getElementById('mainCard');
    if (card && !document.querySelector('.photo-loader')) {
        const loader = document.createElement('div');
        loader.className = 'photo-loader';
        loader.id = 'photoLoader';
        loader.innerHTML = '<div class="loader-spinner"></div>';
        // Profile img üzerine gelecek şekilde ekle
...
function initTouchInteractions() {
    ...
    document.addEventListener('touchend', (e) => {
        ...
        if(Math.abs(diff) > 50) {
            if(diff > 0) { // Aşağı Kaydır (Sonraki)
                 if(state.stage < 4) { state.stage++; changeStage(); lockScroll(); } 
                 else { triggerBump('bump-up'); lockScroll(400); }
            } else { // Yukarı Kaydır (Önceki)
                 if(state.stage > 0) { state.stage--; changeStage(); lockScroll(); } 
                 else { triggerBump('bump-down'); lockScroll(400); }
            }
        }
...
// DÜZELTİLDİ: Artık 0'dan 4'e dönüyor (En üst 0)
...
// YENİ: Yön bilgisi gönderiliyor (1: İleri, -1: Geri)
function nextPhoto() { state.photoIndex = (state.photoIndex + 1) % CONFIG.photos.length; updatePhoto(1); }
function prevPhoto() { state.photoIndex = (state.photoIndex - 1 + CONFIG.photos.length) % CONFIG.photos.length; updatePhoto(-1); }

// GÜNCELLENMİŞ UPDATE PHOTO: Loader ve Animasyon
function updatePhoto(direction = 1) { 
    const img = document.getElementById("profileImg");
    const loader = document.getElementById("photoLoader");
    
    // Yükleme efektini göster
    if(loader) loader.classList.add('active');
    
    // Eski resme çıkış animasyonu uygula (yöne göre)
    if(direction === 1) img.classList.add("slide-left");
    else img.classList.add("slide-right");

    // Yeni resmi arka planda yükle
    const tempImg = new Image();
    tempImg.src = CONFIG.photos[state.photoIndex];
    
    tempImg.onload = () => {
        // Resim yüklendiğinde değiştir
        img.src = tempImg.src;
        
        // Çıkış animasyonunu temizle ve giriş animasyonunu ekle
        img.classList.remove("slide-left", "slide-right");
        
        // Giriş animasyonu
        if(direction === 1) img.classList.add("slide-in-right");
        else img.classList.add("slide-in-left");

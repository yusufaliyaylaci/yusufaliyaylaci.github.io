...
function fetchWeather(lat, lon, cityName = "Konumunuz") {
    fetch(`${CONFIG.weatherApi}?latitude=${lat}&longitude=${lon}&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m&timezone=auto`)
        .then(res => res.json()).then(data => { 
            if(data.timezone) state.timeZone = data.timezone; 
            updateWeatherUI(data.current, cityName); 
        });
}
...
function updateWeatherUI(current, name) {
    ...
    let icon = "fa-cloud", color = "#fff";
    const code = current.weather_code;
    if (code === 0) { icon = "fa-sun"; color = "#ffd700"; }
    else if (code <= 3) { icon = "fa-cloud-sun"; color = "#d4d4d4"; }
    else if (code <= 67) { icon = "fa-cloud-rain"; color = "#00bfff"; }
    else if (code <= 77) { icon = "fa-snowflake"; color = "#fff"; }
